...
from wireless_display_handler import launch_w_display_manager
...
class GUI(tk.Tk):
    ...
    def __init__(self, title='3D Render With Wireless Display Support', min_size=(MIN_WIDTH, MIN_HEIGHT)):
        ''''''
        super().__init__()
        ...
        self._changed = True # A flag used to only redraw the object when a change occurred
        ...
        self._fill_color_holder = "#EEEEEE"
        self._line_color_holder = "#0083EB"
    ...
    def __create_zoom_slider(self):
        ...
        # Bind the touchpad events for zooming
        self._canvas.bind('<MouseWheel>', self.__zoom_with_touchpad)

    def __zoom_with_touchpad(self, event):
        '''Callback for zooming with touchpad'''
        if event.delta > 0:  # Scrolling up
            self._zoom_slider.set(self._zoom_slider.get() - 1)  # Adjust the step as needed
    ...
    def __create_screenshot_button(self):
        ...
        ttk.Button(self, text="Open .obj File", command=self.__read_file).place(relx=self.COMMON_X, rely=0.815, relheight=0.05, relwidth=0.1, anchor="ne")
    ...
    def __take_screenshot(self):
        transfer_to_remote = messagebox.askyesno("Transfer to Wireless Display", "Would you like to transfer the screenshot to the wireless display?")
        
        if transfer_to_remote:
            ...
            screenshot_resized = screenshot.resize((320, 240))  # Resize image to fit 320x240 bitmap
            launch_w_display_manager(self, screenshot_resized)
            
        else:
            # Proceed with taking the screenshot and saving it locally
            x = self.winfo_rootx() + self._canvas.winfo_x()
            y = self.winfo_rooty() + self._canvas.winfo_y()
            x1 = x + self._canvas.winfo_width()
            y1 = y + self._canvas.winfo_height()
